...
public class SysPostController {
    ...
    public ResultBase<SysPost> findById(@RequestParam(name = "id") Long id) {
        ResultBase<SysPost> baseResult = null;
        boolean present = sysPostService.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(sysPostService.findById(id).get());
        } else {
    ...
    public ResultBase<List<SysPost>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<SysPost> sysPosts = sysPostService.findAll(sort);
        return ResultUtils.success(sysPosts);
    }
    ...
    public ResultBase<Page<SysPost>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<SysPost> sysPosts = sysPostService.findByPage(pageable);
        return ResultUtils.success(sysPosts);
    }
    ...
    public ResultBase<SysPost> saveOne(@RequestBody SysPost sysPost) {
        SysPost sysPosts = sysPostService.saveOne(sysPost);
        return ResultUtils.success(sysPosts);
    }
    ...
    public ResultBase<SysPost> updateByObj(@RequestBody SysPost sysPost) {
        SysPost sysPosts = sysPostService.updateByObj(sysPost);
        return ResultUtils.success(sysPosts);
    }
    ...
    public ResultBase<SysPost> deleteById(@RequestParam(name = "id") Long id) {
