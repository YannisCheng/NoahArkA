...
public class SysDeptController {
    ...
    public ResultBase<SysDept> findById(@RequestParam(name = "id") Long id) {
        ResultBase<SysDept> baseResult = null;
        boolean present = sysDeptService.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(sysDeptService.findById(id).get());
        } else {
    ...
    public ResultBase<List<SysDept>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<SysDept> sysDepts = sysDeptService.findAll(sort);
        return ResultUtils.success(sysDepts);
    }
    ...
    public ResultBase<Page<SysDept>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<SysDept> sysDepts = sysDeptService.findByPage(pageable);
        return ResultUtils.success(sysDepts);
    }
    ...
    public ResultBase<SysDept> saveOne(@RequestBody SysDept sysDept) {
        SysDept sysDepts = sysDeptService.saveOne(sysDept);
        return ResultUtils.success(sysDepts);
    }
    ...
    public ResultBase<SysDept> updateByObj(@RequestBody SysDept sysDept) {
        SysDept sysDepts = sysDeptService.updateByObj(sysDept);
        return ResultUtils.success(sysDepts);
    }
    ...
    public ResultBase<SysDept> deleteById(@RequestParam(name = "id") Long id) {
