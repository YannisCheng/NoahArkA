...
public class Table2Controller {
    ...
    public ResultBase<Table2> findById(@RequestParam(name = "id") Integer id) {
        ResultBase<Table2> baseResult = null;
        boolean present = table2Service.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(table2Service.findById(id).get());
        } else {
    ...
    public ResultBase<Table2> findByIdCard(@RequestParam(name = "idCard") String idCard) {
        ResultBase<Table2> baseResult = null;
        boolean present = table2Service.findByIdCard(idCard).isPresent();
        if (present) {
            baseResult = ResultUtils.success(table2Service.findByIdCard(idCard).get());
        } else {
            baseResult = ResultUtils.errorData();
        }
        return baseResult;
    ...
    public ResultBase<Table2> findByStuId(@RequestParam(name = "stuId") String stuId) {
        ResultBase<Table2> baseResult = null;
        boolean present = table2Service.findByStuId(stuId).isPresent();
        if (present) {
            baseResult = ResultUtils.success(table2Service.findByStuId(stuId).get());
        } else {
            baseResult = ResultUtils.errorData();
        }
        return baseResult;
    ...
    public ResultBase<List<Table2>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<Table2> table2s = table2Service.findAll(sort);
        return ResultUtils.success(table2s);
    }
    ...
    public ResultBase<Page<Table2>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<Table2> table2s = table2Service.findByPage(pageable);
        return ResultUtils.success(table2s);
    }
    ...
    public ResultBase<Table2> saveOne(@RequestBody Table2 table2) {
        Table2 table2s = table2Service.saveOne(table2);
        return ResultUtils.success(table2s);
    }
    ...
    public ResultBase<Table2> updateByObj(@RequestBody Table2 table2) {
        Table2 table2s = table2Service.updateByObj(table2);
        return ResultUtils.success(table2s);
    }
    ...
    public ResultBase<Table2> deleteById(@RequestParam(name = "id") Integer id) {
