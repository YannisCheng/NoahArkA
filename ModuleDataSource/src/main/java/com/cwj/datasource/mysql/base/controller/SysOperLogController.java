...
public class SysOperLogController {
    ...
    public ResultBase<SysOperLog> findById(@RequestParam(name = "id") Long id) {
        ResultBase<SysOperLog> baseResult = null;
        boolean present = sysOperLogService.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(sysOperLogService.findById(id).get());
        } else {
    ...
    public ResultBase<List<SysOperLog>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<SysOperLog> sysOperLogs = sysOperLogService.findAll(sort);
        return ResultUtils.success(sysOperLogs);
    }
    ...
    public ResultBase<Page<SysOperLog>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<SysOperLog> sysOperLogs = sysOperLogService.findByPage(pageable);
        return ResultUtils.success(sysOperLogs);
    }
    ...
    public ResultBase<SysOperLog> saveOne(@RequestBody SysOperLog sysOperLog) {
        SysOperLog sysOperLogs = sysOperLogService.saveOne(sysOperLog);
        return ResultUtils.success(sysOperLogs);
    }
    ...
    public ResultBase<SysOperLog> updateByObj(@RequestBody SysOperLog sysOperLog) {
        SysOperLog sysOperLogs = sysOperLogService.updateByObj(sysOperLog);
        return ResultUtils.success(sysOperLogs);
    }
    ...
    public ResultBase<SysOperLog> deleteById(@RequestParam(name = "id") Long id) {
