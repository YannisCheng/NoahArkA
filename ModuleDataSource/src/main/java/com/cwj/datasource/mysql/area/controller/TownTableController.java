...
public class TownTableController {
    ...
    public ResultBase<TownTable> findById(@RequestParam(name = "id") Integer id) {
        ResultBase<TownTable> baseResult = null;
        boolean present = townTableService.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(townTableService.findById(id).get());
        } else {
    ...
    public ResultBase<List<TownTable>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<TownTable> townTables = townTableService.findAll(sort);
        return ResultUtils.success(townTables);
    }
    ...
    public ResultBase<Page<TownTable>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<TownTable> townTables = townTableService.findByPage(pageable);
        return ResultUtils.success(townTables);
    }
    ...
    public ResultBase<TownTable> saveOne(@RequestBody TownTable townTable) {
        TownTable townTables = townTableService.saveOne(townTable);
        return ResultUtils.success(townTables);
    }
    ...
    public ResultBase<TownTable> updateByObj(@RequestBody TownTable townTable) {
        TownTable townTables = townTableService.updateByObj(townTable);
        return ResultUtils.success(townTables);
    }
    ...
    public ResultBase<TownTable> deleteById(@RequestParam(name = "id") Integer id) {
        townTableService.deleteById(id);
