...
public class SysNoticeController {
    ...
    public ResultBase<SysNotice> findById(@RequestParam(name = "id") Integer id) {
        ResultBase<SysNotice> baseResult = null;
        boolean present = sysNoticeService.findById(id).isPresent();
        if (present) {
            baseResult = ResultUtils.success(sysNoticeService.findById(id).get());
        } else {
    ...
    public ResultBase<List<SysNotice>> findAll() {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        List<SysNotice> sysNotices = sysNoticeService.findAll(sort);
        return ResultUtils.success(sysNotices);
    }
    ...
    public ResultBase<Page<SysNotice>> findByPage(@RequestParam(name = "page") int page, @RequestParam(name = "size") int size) {
        // 通过主键进行排序
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<SysNotice> sysNotices = sysNoticeService.findByPage(pageable);
        return ResultUtils.success(sysNotices);
    }
    ...
    public ResultBase<SysNotice> saveOne(@RequestBody SysNotice sysNotice) {
        SysNotice sysNotices = sysNoticeService.saveOne(sysNotice);
        return ResultUtils.success(sysNotices);
    }
    ...
    public ResultBase<SysNotice> updateByObj(@RequestBody SysNotice sysNotice) {
        SysNotice sysNotices = sysNoticeService.updateByObj(sysNotice);
        return ResultUtils.success(sysNotices);
    }
    ...
    public ResultBase<SysNotice> deleteById(@RequestParam(name = "id") Integer id) {
